...
export default function PreviousCycleSection({newSchool, setNewSchool, loggedInUser, isEdit, handleInputInCategory}: { newSchool: School, setNewSchool: Dispatch<SetStateAction<School>>
    ...
    useEffect(() => {
        if (newSchool.edited_school_average_gpa_accepted_previous_cycle.edited_average_overall_gpa_accepted_previous_year.input !== null || newSchool.edited_school_average_gpa_accepted_previous_cycle.edited_average_prerequisite_gpa_accepted_previous_year.input !== null 
            || newSchool.edited_school_average_gpa_accepted_previous_cycle.edited_average_science_gpa_accepted_previous_year.input !== null || newSchool.edited_school_average_gpa_accepted_previous_cycle.edited_average_bcp_gpa_accepted_previous_year.input !== null) {
                setHasInputs(true)
            } else {
                setHasInputs(null)
            }
    ...
    return (
        ...
        <div className={`mt-28 flex justify-start items-start gap-3 w-full`}>
            <div className={`grow relative max-w-[900px] border-2 p-4 block rounded border-[#B4B4B4]`}>
                <label className="absolute top-[-16px] text-xl bg-white flex justify-start items-center">Average GPA Accepted Previous Cycle<PiCheckCircle className={`h-5 w-5 ml-[2px] ${!hasInputs ? 'text-[#4FC769]' : 'text-[#B4B4B4]'}`} /><PiWarningCircle className={`h-5 w-5 ml-[2px] ${hasInputs ? 'text-[#F06A6A]' : 'text-[#B4B4B4]'}`}/></label>
                    ...
                    <>
                        <div className={`${i === 0 ? 'mt-8' : 'mt-12'} mx-4 relative max-w-[900px] p-4 block rounded border-[#545454] border-2`}>
                            ...
                            <div className='flex justify-start items-center gap-4'>
                                <InputFieldsGroup loggedInUser={loggedInUser} input={field.input} isEditMode={field.isEditMode} originalInput={originalField.input} name={gpa.value} category='school_average_gpa_accepted_previous_cycle' handleInput={handleInputInCategory}/>
