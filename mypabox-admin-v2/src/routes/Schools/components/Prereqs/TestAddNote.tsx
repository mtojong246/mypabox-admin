...
export default function TestAddNote({ toggleNotePopup, addNote, editedNote, setEditedNote, updateNote, note }: { 
    toggleNotePopup: (e:any) => void, 
    addNote: (note: Note, isEditedInput?: boolean) => void,
    editedNote: any | null,
    updateNote: (note: Note, isEditedInput?: boolean) => void,
    setEditedNote: Dispatch<SetStateAction<any | null>>,
    note: EditedNote | null,
}) {
    const [ optionalNote, setOptionalNote ] = useState<Note>(defaultNote);
    const [ editedOption, setEditedOption ] = useState<EditedNote | null>(null);

    useEffect(() => {
        if (editedNote) {
            if (note) {
                setEditedOption(editedNote);
    ...
    const addOrEditNote = (e: MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        if (!editedOption && optionalNote.note === '') {
            alert('Please add text to note')
        }  else if (editedOption && editedOption.note === '') {
            alert('Please add text to note')
        } else {
            if (editedNote) {
                updateNote(editedOption ? editedOption : optionalNote, editedOption ? true : false)
            } else {
                addNote(editedOption ? editedOption : optionalNote, editedOption ? true : false)
            }
            toggleNotePopup(e)
                ...
                <div className='w-full max-w-[900px] rounded-lg p-4 bg-white'>
                    ...
                    <div className='w-full mb-8'>
                        ...
                        <div className='ml-4 mb-2'>
                            <input onChange={handleType} value='information' type='radio' className='mr-2' checked={editedOption ? editedOption.type === 'information' : optionalNote.type === 'information'}/>
                            <label>Information</label>
                        </div>
                        <div className='ml-4'>
                            <input onChange={handleType} value='requirement' type='radio' className='mr-2' checked={editedOption ? editedOption.type === 'requirement' : optionalNote.type === 'requirement'}/>
                            <label>Requirement</label>
                        </div>
                    </div>
                    <div className='w-full mb-14'>
                        <label className='font-medium'>Note:</label>
                        <ReactQuill className='mt-2 h-[200px] rounded w-full' theme="snow" onChange={handleNote} value={editedOption ? editedOption.note : optionalNote.note}/>
                    </div>
