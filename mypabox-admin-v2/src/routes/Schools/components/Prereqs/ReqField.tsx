...
export default function ReqField({loggedInUser, input, originalInput, isEditMode, setEditedOption, deleteOption, undoDelete, toggleOptions, setGroupIndex, courses}: {
        ...
        {input !== null && input.length > 0 && (
            <div className={`flex flex-col justify-center items-center gap-3 ${input.length ? 'mt-3' : 'mt-0'}`}>
                {courses && input.map((course, i) => {
                    ...
                    const originalCourse = originalGroup && courses.find(c => c.unique_id === originalGroup.school_required_course_id)
                    if (selectedCourse) {
                        return (
                            <div className={`py-2 pr-2 pl-3 border ${course.isNew ? 'border-orange-600' : 'border-[#545454]'}  rounded w-full`}>
                                <div className='flex justify-between items-start w-full'>
                                    <div>
                                        ...
                                        {originalGroup && (originalGroup.school_required_course_id !== course.school_required_course_id || originalGroup.school_required_course_lab !== course.school_required_course_lab || originalGroup.school_required_course_credit_hours !== course.school_required_course_credit_hours
                                            || originalGroup.school_required_course_lab_preferred !== course.school_required_course_lab_preferred || originalGroup.school_required_course_quarter_hours !== course.school_required_course_quarter_hours) && (
                                            <p className={`font-bold line-through`}>{originalCourse?.course_name}
                                                <span className='font-semibold text-sm inline-block pl-3 text-[#6A6A6A]'>
                                                    {`(${originalGroup.school_required_course_lab ? 'with lab' : 'without lab'}
                                                    ${!originalGroup.school_required_course_lab && originalGroup.school_required_course_lab_preferred ? ' / lab preferred' : ''}  
                                                    / ${originalGroup.school_required_course_credit_hours} credit hours 
                                                    / ${originalGroup.school_required_course_quarter_hours} quarter hours)`}                                   
                                                </span>
                                            </p>
                                            )}
                                    </div>
