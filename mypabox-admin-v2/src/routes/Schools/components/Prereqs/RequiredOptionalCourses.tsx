...
export default function RequiredOptionalCourses({ newSchool, setNewSchool, loggedInUser, isEdit }: { 
    ...
    return (
        ...
        {((loggedInUser.permissions.canVerify && newSchool.edited_school_prereq_required_optional_courses.input !== null) || (!loggedInUser.permissions.canVerify && !newSchool.edited_school_prereq_required_optional_courses.isEditMode)) && <div className='absolute top-0 bottom-0 right-0 left-0 bg-[#999999] opacity-50 z-10'></div>}
        <label className="z-20 absolute top-[-16px] text-xl bg-white flex justify-start items-center">Required Optional Courses<PiCheckCircle className={`h-5 w-5 ml-[2px] ${!hasInputs ? 'text-[#4FC769]' : 'text-[#B4B4B4]'}`} /><PiWarningCircle className={`h-5 w-5 ml-[2px] ${hasInputs ? 'text-[#F06A6A]' : 'text-[#B4B4B4]'}`}/></label> 
