...
export default function AddRecommendedOption({ newSchool, setNewSchool, toggleOptions, editedOption, setEditedOption, groupIndex }: { 
    newSchool: School, 
    ...
    const addOrUpdateOption = (e:MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        if (!options.school_recommended_optional_group_evaluator_title.length) {
            ...
            <div className='fixed bg-[rgba(0,0,0,0.2)] top-0 left-0 right-0 bottom-0 flex justify-center items-center p-10'>
                <div className='relative w-full max-w-[900px] max-h-[700px] overflow-y-scroll rounded-lg p-4 bg-white'>
                    ...
                    <div className='w-full flex justify-end items-center gap-3'>
                        ...
                        <button onClick={addOrUpdateOption} className='text-xl border-2 border-[#4573D2] bg-[#4573D2] text-white font-medium px-5 h-[50px] rounded-md'>{editedOption ? 'Edit' : 'Add'} Option</button>
                    </div>
                </div>
            </div>
        </div>
