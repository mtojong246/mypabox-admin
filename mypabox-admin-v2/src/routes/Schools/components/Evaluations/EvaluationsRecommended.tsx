...
export default function EvaluationsRecommended({ newSchool, setNewSchool }: { newSchool: School, setNewSchool: Dispatch<SetStateAction<School>>}) {
    ...
    const addEvaluator = (e: MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        if (newSchool.school_evaluations_recommended.school_recommended_evaluator_title?.includes(evaluator)) return;
        if (!evaluator) return;
        setNewSchool({
            ...newSchool,
            school_evaluations_recommended: {
                ...newSchool.school_evaluations_recommended,
                school_recommended_evaluator_title: newSchool.school_evaluations_recommended.school_recommended_evaluator_title!.concat(evaluator),
            }
        })
    };
    ...
    const deleteEvaluator = (e: MouseEvent<HTMLButtonElement>, index: number) => {
        ...
        setNewSchool({
            ...
            school_evaluations_recommended: {
                ...newSchool.school_evaluations_recommended,
                school_recommended_evaluator_title: newSchool.school_evaluations_recommended.school_recommended_evaluator_title!.filter((e,i) => i !== index)
            }
        })
    }
    ...
    const addNote = (note: Note) => {
        setNewSchool({
            ...
            school_evaluations_recommended: {
                ...newSchool.school_evaluations_recommended,
                school_evaluations_recommended_notes: newSchool.school_evaluations_recommended.school_evaluations_recommended_notes.concat(note)
            }
        })
    };
    ...
    const updateNote = (note: Note) => {
        setNewSchool({
            ...
            school_evaluations_recommended: {
                ...newSchool.school_evaluations_recommended,
                school_evaluations_recommended_notes: newSchool.school_evaluations_recommended.school_evaluations_recommended_notes.map((n,i) => {
                    if (i === index) {
                        return { ...note }
                    } else {
                        return { ...n }
    ...
    const deleteNote = (e: MouseEvent<HTMLButtonElement>, index: number) => {
        ...
        setNewSchool({
            ...
            school_evaluations_recommended: {
                ...newSchool.school_evaluations_recommended,
                school_evaluations_recommended_notes: newSchool.school_evaluations_recommended.school_evaluations_recommended_notes.filter((n,i) => i !== index)
            }
        })
    }
    ...
    return (
        ...
        <div className={`mt-20 relative max-w-[900px] border p-5 block rounded-lg border-[#B4B4B4]`}>
            ...
            {newSchool.school_evaluations_recommended.input && (
                <>
                    ...
                    <div className={`mt-12 mx-4 relative max-w-[900px] p-5 block rounded-lg border-[#545454] border`}>
                        ...
                        {newSchool.school_evaluations_recommended.school_recommended_evaluator_title && (
                            <div className={`flex flex-col justify-center items-center gap-3 ${newSchool.school_evaluations_recommended.school_recommended_evaluator_title.length ? 'mt-3' : 'mt-0'}`}>
                            {newSchool.school_evaluations_recommended.school_recommended_evaluator_title.map((opt, i) => {
                                return (
            ...
            <div className={`${newSchool.school_evaluations_recommended.input ? 'mx-5 mb-5' : 'mx-0 mb-0'}`}>
            ...
            {newSchool.school_evaluations_recommended.school_evaluations_recommended_notes && (
            <div className={`flex flex-col justify-center items-center gap-3 ${newSchool.school_evaluations_recommended.school_evaluations_recommended_notes.length ? 'mt-3' : 'mt-0'}`}>
                {newSchool.school_evaluations_recommended.school_evaluations_recommended_notes.map((note, i) => (
