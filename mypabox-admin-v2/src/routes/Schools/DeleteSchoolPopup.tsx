...
export default function DeleteSchoolPopup({toggleDelete, schoolToDelete}: {toggleDelete: (e:any) => void, schoolToDelete: { name: string, id: string }}) {
    ...
    const handleDelete = async (e:any) => {
        e.preventDefault();
        setIsLoading(true)
        const school = newSchools.find(s => s.id === schoolToDelete.id);
        if (school) {
            try {
                await deleteUpdatedSchoolDoc(school.id);
                dispatch(deleteNewSchool(school));
                toggleDelete(e);
                // await deleteSchoolDoc(school.id.toString());
                // const newSchools = schools.filter(s => s.id !== school.id);
                // dispatch(setSchools(newSchools));
        ...
        <div className='fixed top-0 left-0 right-0 bottom-0 z-50'>
            ...
            <div className='w-full max-w-[600px] rounded-lg p-4 bg-white absolute top-[50%] left-[50%] transform translate-x-[-50%] translate-y-[-50%] z-30'>
                <div className='w-full flex justify-between items-start gap-8'>
                    <p className='text-xl font-bold'>Delete <q>{schoolToDelete.name}</q>?</p>
                ...
                <div className='mt-4 w-full flex justify-end items-center gap-4'>
                    ...
                    <button onClick={(e:any) => handleDelete(e)} className='rounded w-[119px] h-[40px] bg-[#F06A6A] text-white font-medium flex justify-center items-center'>
