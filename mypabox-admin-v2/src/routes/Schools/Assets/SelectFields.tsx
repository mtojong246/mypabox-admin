...
export default function SelectFields({loggedInUser, isEdit, input, originalInput, isEditMode, name, category, options, handleSelect, label, originalLabel, handleSelectInArray, index, isBlank}: {
    ...
    return (
        ...
        <div className='flex flex-col justify-start items-start gap-3 grow'>
            {input !== null && (
                 <Select className="grow focus:outline-none rounded w-full"
                 options={options} onChange={(e:any) => {index !== undefined && handleSelectInArray !== undefined ? handleSelectInArray(e, category, index, isEditMode) :  handleSelect(e, category, isEditMode)}} 
                 value={{value: input, label: label ? label : input}}/>
            )}

            {isOriginalFieldShown && (
                <Select isDisabled={isOriginalInputDisabled} className="grow focus:outline-none rounded w-full"
                options={options} onChange={(e:any) => {index !== undefined && handleSelectInArray !== undefined ? handleSelectInArray(e, category, index, false) :  handleSelect(e, category, false)}} 
                value={{value: originalInput, label: originalLabel ? originalLabel : originalInput}}/>
            )}
        </div>
