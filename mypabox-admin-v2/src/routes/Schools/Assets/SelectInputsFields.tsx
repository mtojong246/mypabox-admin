        ...
        {loggedInUser.permissions.canVerify ? (
            ...
            <div className='flex flex-col justify-start items-start gap-3 grow'>
                <div className='flex justify-start items-center gap-3 w-full'>
                    ...
                    <Select isDisabled options={options} value={duration ? {value: duration, label: duration} : null} className="grow focus:outline-none"/>
                </div>
                <div className='flex justify-start items-center gap-3 w-full'>
                    ...
                    <Select isDisabled options={options} value={originalDuration ? {value: originalDuration, label: originalDuration} : null} className="grow focus:outline-none"/>
                </div>
            </div>
            ) : (
            <div className='flex justify-start items-center gap-3 grow'>
                ...
                <Select options={options} value={originalDuration ? {value: originalDuration, label: originalDuration} : null} onChange={(e:any) => handleSelect(e, name, false)} className="grow focus:outline-none"/>
            </div>
            )}
            </>
        ) : (
            <div className='flex flex-col justify-start items-start gap-3 grow'>
                {(input !== null || isEditMode) && <div className='flex justify-start items-center gap-3 w-full'>
                    ...
                    <Select onChange={(e:any) => handleSelect(e, name, true)} options={options} value={duration ? {value: duration, label: duration} : null} className="grow focus:outline-none"/>
                </div>}
